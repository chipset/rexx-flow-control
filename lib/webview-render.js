...
function renderGraphHtml(
  ...
  const appClass = defaultViewMode === "detailed" ? "app detailed" : "app";
...
<body>
  ...
  <script nonce="${scriptNonce}">
    ...
    function computeLayout() {
      const densityScale = { compact: .85, regular: 1, comfy: 1.18 }[state.density] || 1;
      ...
      toolbar.append(
        ...
        el('label', { class: 'search' }, [
          ...
          el('input', {
            ...
            oninput: (event) => { state.search = event.target.value; render(); }
          })
        ]),
        segmented(['tree', 'layered', 'radial'], state.layout, (value) => { state.layout = value; render(true); }),
        segmented(['compact', 'regular', 'comfy'], state.density, (value) => { state.density = value; render(true); })
      );
      root.append(toolbar);
    }
    ...
    function renderGraphOnly() {
      ...
      REX.FUNCTIONS.forEach((fn) => {
        ...
        const node = el('button', {
          class: 'node' + (state.selectedId === fn.id ? ' selected' : '') + (hl && state.selectedId !== fn.id ? ' highlight' : '') + (dim ? ' dim' : '') + (hidden ? ' hidden' : ''),
          type: 'button',
          title: fn.sig,
          style: {
            left: pos.x + 'px',
          ...
          onmouseenter: () => { state.hoverId = fn.id; renderGraphOnly(); },
          onmouseleave: () => { state.hoverId = null; renderGraphOnly(); },
          onclick: (event) => { event.stopPropagation(); setSelected(fn.id); }
        }, [
    ...
    function renderLegend(root) {
      ...
      if (state.legendOpen) {
        legend.append(el('div', { class: 'legend-body' }, REX.MODULES.map((mod) => el('div', {
          class: 'legend-row',
